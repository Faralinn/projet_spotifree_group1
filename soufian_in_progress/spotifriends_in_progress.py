...
class ServerSocket():
    ...
    def connexion(self,client_soc):
        ...
        if not self.check :
            ...
            if self.check :
                ...
                self.send_msg(self.client_soc,self.msg)
                self.next_step="exit"
        return(self.next_step,self.username)
    ...
    def creation(self, client_soc):
        ...
        if self.sign_up != "2" :
            ...
            sql.insertion("identifiants","pseudo, mot_de_pass",self.data)
    ...
    def login(self,client_soc) :
        ...
        while self.next_step=="menu" or self.next_step == "connexion":  
            if self.next_step == "connexion" :
                (self.next_step,self.username)=self.connexion(self.client_soc)
            else :
